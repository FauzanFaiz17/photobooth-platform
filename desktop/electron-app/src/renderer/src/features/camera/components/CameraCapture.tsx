...
export default function CameraCapture({
    ...
    const { stage, countdown, currentShotIndex, start, continueAfterReview, retakeCurrent, isActive } = useCaptureSequence({
        totalShots,
        ...
        <div className="flex h-full flex-col items-center justify-center gap-4">
            ...
            {stage === "review" && lastCaptured && (
                <div className="flex flex-col items-center gap-3 text-white">
                    <img src={lastCaptured.dataUrl} style={{ filter: cssFilter }} className="h-48 rounded-lg" />
                    <p>Foto {currentShotIndex + 1}: sudah sesuai?</p>
                    <div className="flex gap-3"><Button onClick={retakeCurrent} className="bg-slate-500">Ulangi</Button><Button onClick={handleContinue}>Lanjutkan</Button></div>
                </div>
            )}
            ...
            {stage === "done" && (
                ...
                <p className="text-white">Selesai! Menyiapkan preview...</p>
