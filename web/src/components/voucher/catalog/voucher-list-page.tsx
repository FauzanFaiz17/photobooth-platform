import { Building2, RefreshCw, Ticket } from "lucide-react";
...
export function VoucherListPage(): ReactElement {
  ...
  const visiblePartners = useMemo(() => {
    ...
    return partners.filter((partner) =>
      `${partner.companyName} ${partner.brandName ?? ""}`
  ...
  return (
    <div className="min-w-0 space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      {state === "error" && (
        <Card>
          <CardContent className="grid min-h-48 place-items-center gap-3 p-6 text-center">
            <div>
              <p className="font-medium">Data Partner gagal dimuat</p>
      ...
      {state === "success" && visiblePartners.length === 0 && (
        <Card>
          ...
          {visiblePartners.map((partner) => (
            ...
            >
              ...
              <CardContent>
                <Button
                  className="w-full"
                  render={<Link to={`/voucher/${partner.id}`} />}
                >
                  <Ticket aria-hidden="true" /> Kelola Voucher
                </Button>
              </CardContent>
            </Card>
          ))}
