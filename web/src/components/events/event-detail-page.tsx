      ...
      {loadState === "success" && event && (
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Card><CardHeader><CardTitle>Booth dan Partner</CardTitle><CardDescription>Lokasi Event serta pembuatnya.</CardDescription></CardHeader><CardContent><dl className="grid gap-5 sm:grid-cols-2"><DetailItem label="Booth">{event.booth.name}</DetailItem><DetailItem label="Lokasi">{event.booth.location || "—"}</DetailItem><DetailItem label="Partner">{event.partner.brand_name || event.partner.company_name}</DetailItem><DetailItem label="Dibuat oleh">{event.creator.name}</DetailItem></dl></CardContent></Card>
          </div>

          <Card><CardHeader><CardTitle>Snapshot konfigurasi</CardTitle><CardDescription>Konfigurasi disalin ketika Event dibuat sehingga perubahan profile tidak mengubah Event ini.</CardDescription></CardHeader><CardContent><dl className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4"><DetailItem label="Template">{event.template_snapshot.name} · v{event.template_snapshot.version}</DetailItem><DetailItem label="Filter">{event.filter_snapshot.name} · {event.filter_snapshot.intensity}%</DetailItem><DetailItem label="Camera">Profile #{event.camera_snapshot.camera_profile_id} · ISO {event.camera_snapshot.iso || "—"} · {event.camera_snapshot.burst_count} foto</DetailItem><DetailItem label="Printer">{event.printer_snapshot.printer_name} · {event.printer_snapshot.copies} salinan · {event.printer_snapshot.paper_size}</DetailItem></dl></CardContent></Card>
