...
/** Backend membatasi per_page pada rentang 5–100. */
...
interface PartnerListPaginationProps {
  readonly meta: PaginationMeta
  readonly onPageChange: (page: number) => void
  readonly onPageSizeChange: (pageSize: number) => void
}

export function PartnerListPagination({
  meta,
  onPageChange,
  onPageSizeChange,
}: PartnerListPaginationProps) {
  ...
  return (
    <div className="flex flex-col gap-3 border-t px-4 pt-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
      <p className="text-sm tabular-nums text-muted-foreground">
        {meta.from ?? 0}–{meta.to ?? 0} dari {meta.total} partner
      </p>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <SelectTrigger
            className="w-32"
            aria-label="Jumlah partner per halaman"
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
