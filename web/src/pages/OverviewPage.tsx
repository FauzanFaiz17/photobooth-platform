...
const boothStatusLabels: Record<BoothStatus, string> = {
  active: "Aktif",
...
function accessNote(
  ...
  if (error instanceof ApiError && error.status === 403)
    return `${label} tidak tersedia karena role belum memiliki permission.`;
  ...
  useEffect(() => {
    ...
    async function loadOverview() {
      ...
      const results = await Promise.allSettled([
        getBooths(
          accessToken,
          { per_page: 100, sort: "created_at", direction: "desc" },
          controller.signal,
        ),
        getEvents(
          accessToken,
          { status: "ongoing", per_page: 5 },
          controller.signal,
        ),
        getEvents(
          accessToken,
          { sort: "event_date", direction: "desc", per_page: 5 },
          controller.signal,
        ),
        superAdmin
          ? getPartners(accessToken, { per_page: 5 }, controller.signal)
      ...
      const boothsResponse =
        boothsResult.status === "fulfilled" ? boothsResult.value : null;
      ...
      const partnersResponse =
        partnersResult.status === "fulfilled" ? partnersResult.value : null;
      ...
      setData({
        partnerTotal: superAdmin
          ? (partnersResponse?.meta.total ?? null)
          : user?.partner
            ? 1
            : 0,
        boothTotal: boothsResponse?.meta.total ?? null,
        activeBooths: boothsResponse
          ? boothsResponse.data.filter((booth) => booth.status === "active")
              .length
          : null,
        deviceTotal: devicesResponse?.meta.total ?? null,
        onlineDevices: devicesResponse
          ? devices.filter((device) => device.presence_status === "online")
              .length
          : null,
        staleDevices: devicesResponse
          ? devices.filter((device) => device.presence_status === "stale")
              .length
          : null,
        offlineDevices: devicesResponse
          ? devices.filter((device) => device.presence_status === "offline")
              .length
          : null,
      ...
      {loadState === "loading" ? (
        ...
        <>
          <section
            ...
            aria-label="Statistik backend"
          >
            <StatCard
              icon={Building2}
              label="Partner/Kiosk"
              value={data.partnerTotal?.toLocaleString("id-ID") ?? "—"}
              description={
                superAdmin ? "Partner terdaftar" : "Partner akun ini"
              }
            />
            <StatCard
              icon={Monitor}
            ...
            <StatCard
              icon={CalendarDays}
              label="Event aktif"
              value={data.ongoingEvents?.toLocaleString("id-ID") ?? "—"}
              description="Event berstatus berlangsung"
            />
            <StatCard
              icon={Cpu}
              label="Perangkat"
              value={data.deviceTotal?.toLocaleString("id-ID") ?? "—"}
          ...
          {data.notes.length > 0 && (
            <Card>
              <CardContent className="flex items-start gap-3 p-4">
                ...
                />
                <div>
                  <p className="font-medium">Keterangan data</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Booth terbaru</CardTitle>
              <CardDescription>
                Status Booth dan jumlah perangkat terdaftar.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {data.booths.length === 0 ? (
                <p className="py-12 text-center text-sm text-muted-foreground">
                  Data Booth belum tersedia.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {data.booths.slice(0, 8).map((booth) => (
                        <TableRow key={booth.id}>
                          <TableCell>
                            <div className="font-medium">{booth.name}</div>
                            <div className="text-xs text-muted-foreground">
                              {booth.location || "Lokasi belum diisi"}
                            </div>
                          </TableCell>
                          <TableCell>
                            {booth.partner.brand_name ||
                              booth.partner.company_name}
                          </TableCell>
                          <TableCell>
                            <Badge
                              variant={
                            ...
                            >
                              {boothStatusLabels[booth.status]}
                            </Badge>
                          </TableCell>
                          <TableCell>{booth.devices_count ?? 0}</TableCell>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Jadwal Event dari endpoint `/api/v1/events`.
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <TableBody>
                      {data.events.map((event) => (
                        <TableRow key={event.id}>
                          ...
                          <TableCell>{event.booth.name}</TableCell>
                          <TableCell>
                            <Badge
                              variant={
