...
export interface PartnerSubscriptionRecord {
  ...
  partner: { id: number; company_name: string; slug: string }
...
export function isPartnerSubscriptionRecord(value: unknown): value is PartnerSubscriptionRecord { return isRecord(value) && isNumber(value.id) && isSubscriptionStatus(value.status) && typeof value.starts_at === "string" && typeof value.ends_at === "string" && typeof value.auto_renew === "boolean" && isNullableString(value.cancelled_at) && typeof value.is_current === "boolean" && isNumber(value.remaining_days) && isRecord(value.partner) && isNumber(value.partner.id) && typeof value.partner.company_name === "string" && typeof value.partner.slug === "string" && isRecord(value.plan) && isNumber(value.plan.id) && typeof value.plan.name === "string" && isBillingCycle(value.plan.billing_cycle) && isNumber(value.plan.price) && isNumber(value.plan.max_booths) && isNumber(value.plan.max_devices) && isNumber(value.plan.max_operators) && typeof value.created_at === "string" && typeof value.updated_at === "string" }
