...
export interface PrintOptionForm {
  id: number;
  paper_size: "2r" | "4r";
...
export type PrintOptionField = Exclude<keyof PrintOptionForm, "id">;
export type PrintOptionErrors = Partial<
...
function isPartner(value: unknown): value is EventPartner {
  return (
    ...
    (value.brand_name === undefined || isNullableString(value.brand_name))
  );
}
...
function isBooth(value: unknown): value is EventBooth {
  return (
    ...
    (value.location === undefined || isNullableString(value.location)) &&
...
function isCreator(value: unknown): value is EventCreator {
  return (
    ...
    isNumber(value.id) &&
    typeof value.name === "string" &&
    (value.email === undefined || isNullableString(value.email))
  );
}
...
function isPrinterSnapshot(value: unknown): value is EventPrinterSnapshot {
  return (
    isRecord(value) &&
    isNumber(value.id) &&
    isNumber(value.printer_profile_id) &&
  ...
  return (
    ...
    (value.templates === undefined ||
      (Array.isArray(value.templates) &&
        value.templates.every(isTemplateSnapshot))) &&
    isFilterSnapshot(value.filter) &&
    (value.filters === undefined ||
      (Array.isArray(value.filters) &&
        value.filters.every(isFilterSnapshot))) &&
    (value.print_options === undefined ||
