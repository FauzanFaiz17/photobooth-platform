...
class PrintJobController extends Controller
{
    ...
    public function index(PollPrintJobsRequest $r)
    {
        return ApiResponse::success(PrintJobResource::collection($this->service->poll($r->user(), $r->validated())), 'Print queue retrieved.');
    }

    public function update(UpdatePrintJobRequest $r, PrintJob $printJob)
    {
        return ApiResponse::success(new PrintJobResource($this->service->transitionForDesktop($printJob, $r->user(), $r->validated())), 'Print job updated.');
    }
    ...
    public function media(PollPrintJobsRequest $r, PrintJob $printJob): StreamedResponse
    {
        ...
        if ($stream === false) {
            abort(404, 'Printable media file is unavailable.');
        }
